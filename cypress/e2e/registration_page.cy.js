describe('User registration user flows', () => {
  ...
  it.only('Should not submit the form unless all fields contain valid inputs', () => {
    ...
    cy.get('input[name="password"]').type('Test1234&');
    cy.get('button[type="submit"]').click();
    cy.contains('.inline-error', 'Passwords must match', { timeout: 2000 }).should('be.visible');
    cy.contains('p', 'Coolio, your account was successfully registered!').should('not.exist');
    cy.get('input[name="password"]').clear();
    cy.get('input[name="confirmPassword"]').type('Test1234&');
    cy.get('button[type="submit"]').click();
    cy.contains('.inline-error', 'Password must be between 8 and 24 characters', { timeout: 2000 }).should(
      'be.visible'
    );
    cy.contains('p', 'Coolio, your account was successfully registered!').should('not.exist');
    cy.get('input[name="username"]').clear();
    cy.get('input[name="password"]').type('Test1234&');
    cy.get('button[type="submit"]').click();
    cy.contains('.inline-error', 'Username must be between 3 and 24 characters', { timeout: 2000 }).should(
      'be.visible'
    );
